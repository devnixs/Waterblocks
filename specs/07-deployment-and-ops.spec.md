...
- [X] OPS-007: The admin nginx config serves SPA assets from `/usr/share/nginx/html` on port 80: long-cache (`max-age=31536000, immutable`) for static `.js/.css/.png/.jpg/.jpeg/.gif/.svg/.ico/.woff/.woff2`, `no-store` cache headers for `/config.js` (so runtime config refreshes), and a SPA fallback `try_files $uri $uri/ /index.html` with `no-store` on the index. Runtime config is rendered by envsubst from `config.js.template` and exposes both `apiBaseUrl` (from `API_BASE_URL`) and `archiveAllWorkspacesEnabled` (from `ARCHIVE_ALL_WORKSPACES_ENABLED`) on `window.__WB_CONFIG__` so the admin UI can hide or show the bulk workspace-archive control without a rebuild. Source: `waterblocks-admin/nginx.conf`, `waterblocks-admin/config.js.template`, `waterblocks-admin/docker-entrypoint.d/99-runtime-config.sh`, `waterblocks-admin/src/config/runtimeConfig.ts`. Confidence: high.
